...
void DIC_Rotaciona_Direita(struct No **R)
{
	...
	*R = e;
	...
	e->altura = MAX( DIC_altura(e->dir), DIC_altura(e->esq)) + 1;
	r->altura = MAX( DIC_altura(r->dir), DIC_altura(r->esq)) + 1;	
}
...
void DIC_Rotaciona_Esquerda(struct No **R)
{
	...
	*R = d;
	...
	d->altura = MAX( DIC_altura(d->dir), DIC_altura(d->esq)) + 1;
	r->altura = MAX( DIC_altura(r->dir), DIC_altura(r->esq)) + 1;
}
...
void DIC_Balanceia_AVL(struct No **R)
{
	...
	if(DIC_altura(r->esq) - DIC_altura(r->dir) > 1)
	{
		if(DIC_altura(r->esq->esq) >= DIC_altura(r->esq->dir))
		{
	printf("dir %s\n", (*R)->I->c);
		...
		{
	printf("esqdir %s\n", (*R)->I->c);
			DIC_Rotaciona_Esquerda(&(r->esq));
			DIC_Rotaciona_Direita(R);
		}
	}
	else if(DIC_altura(r->dir) - DIC_altura(r->esq) > 1)
	{
		if(DIC_altura(r->dir->dir) >= DIC_altura(r->dir->esq))
		{
	printf("esq %s\n", (*R)->I->c);
